...
// ============================================
export async function generateBlogPostImage(title, description = '') {
  const prompt = `Create a professional, engaging blog post featured image for: "${title}"
  ${description ? `Description: ${description}` : ''}
  
  Style: Clean, modern, professional. Use warm colors, natural lighting. Include elements related to solar energy, power, or sustainable living.
  Format: 1200x630 pixels (16:9 ratio).`

  return await generateImage({ prompt, width: 1200, height: 630 })
}
...
export async function generateProductImage(productName, category = '') {
  ...
  Style: Clean white background, studio lighting, professional product photography. The product should be the main focus.
  Format: 1080x1080 pixels (square).`

  return await generateImage({ prompt, width: 1080, height: 1080 })
}
...
async function pollFalGeneration(statusUrl, responseUrl, retries = 30) {
  try {
    ...
    if (retries <= 0) {
      console.error('FAL generation timed out after 30 retries')
      return null
    }
