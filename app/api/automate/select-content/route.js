...
// Content library (same as above - or import from a shared file)
...
export async function GET(request) {
  ...
  try {
    ...
    const availableContent = contentLibrary.filter(content => 
      !postedTitles.includes(content.title)
    )
    
    // If all content is posted, reset and allow all
    ...
    return NextResponse.json({
      ...
      remaining: contentPool.length,
      total: contentLibrary.length
    })
  } catch (error) {
    console.error('Error selecting content:', error.message)
